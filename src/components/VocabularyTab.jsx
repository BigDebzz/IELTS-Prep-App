...
export default function VocabularyTab({ user }) {
  ...
  async function handleLookup(e) {
    e.preventDefault();
    if (!word.trim()) return;
    setLoading(true); setError(''); setResult(null); setSentenceFeedback(null);
    try {
  ...
  async function handleSaveWord() {
    ...
    alert(`"${result.word}" saved to your vocab list.`);
  }
      ...
      {result && (
        <div style={s.card}>
          <div style={s.wordRow}>
            ...
            {result.audioUrl && (
              <button style={s.audioBtn} onClick={() => new Audio(result.audioUrl).play()}>🔊 Pronounce</button>
            )}
          </div>
